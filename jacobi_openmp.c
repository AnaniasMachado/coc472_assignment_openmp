...
int run(double *A, double *b, double *x, double *xtmp)
{
    ...
    do
    {
        // Começo de trecho que pode ser paralizado com OpenMP
        ...
        // Começo de trecho que pode ser paralizado com Open MP
...
void parse_arguments(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (!strcmp(argv[i], "--num_threads") || !strcmp(argv[i], "-nt"))
        {
            if (++i >= argc || (NUM_THREADS = parse_int(argv[i])) <= 0)
            {
                printf("Invalid num_threads\n");
                exit(1);
            }
        }
